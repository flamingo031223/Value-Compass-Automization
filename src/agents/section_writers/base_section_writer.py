...
Use plain ASCII text only.
    ...
    def _writing_constraints(self) -> str:
        ...
        return (
            ...
            "\n"
            "**Title–body qualifier consistency (CRITICAL):**\n"
            "- Finding titles (STABLE Tier A sentences) often contain scale qualifier\n"
            "  words such as 'subtle', 'slight', 'moderate', 'significantly', or\n"
            "  'substantially'. These qualifiers were calibrated to the baseline data.\n"
            "- When the body content of a finding has been ADAPTED or REFRESHED with\n"
            "  new data, you MUST verify that the title qualifier still matches the\n"
            "  actual scale of change shown in the body.\n"
            "- Rule: if the body examples show that most specific examples (model names,\n"
            "  dimension leaders, rankings) have changed compared to what the title\n"
            "  implies, update ONLY the qualifier word in the title while leaving all\n"
            "  other wording intact. Examples:\n"
            "    'subtle preference differences' -> 'notable preference differences'\n"
            "    'significantly outperforms'     -> 'outperforms' or 'moderately outperforms'\n"
            "    'slight gap'                    -> 'narrowing gap'\n"
            "- If the Writing Guidance block contains a 'title_qualifier_note' for a\n"
            "  finding, follow that instruction exactly and update the qualifier word.\n"
            "- KEEP the qualifier unchanged only when the body content confirms it\n"
            "  is still accurate. Never let a STABLE title qualifier contradict the\n"
            "  ADAPT/REFRESH body — that is a factual inconsistency.\n"
            "\n"
            "**New findings:**\n"
