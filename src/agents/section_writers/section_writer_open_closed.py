...
class OpenClosedSectionWriter(BaseSectionWriter):
    ...
    def _section_instruction(self) -> str:
        return (
            ...
            "**Finding 2 — low-scoring open-source model examples (CRITICAL)**\n"
            "When citing specific open-source models as 'low scorers' on Schwartz, you MUST "
            "use ONLY models from the 'open_low_scorers' list provided in Writing Guidance. "
            "These are pre-filtered to Type=Open and ranked by Schwartz total score ascending "
            "(lowest first). Only use a model as a 'low-scoring example' if "
            "'below_median': true for that model. "
            "Do NOT pick models from the full ranked list — that list includes proprietary "
            "models and will produce incorrect examples.\n"
        )
